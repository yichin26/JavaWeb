...
public class Utils {
	...
	public static SortedMap[] parseFood(String json) {
		...
		for(int i=0;i<root.length();i++) {
			...
			map.put("tel", food.getString("Tel").length()==0?"0":food.getString("Tel"));
			map.put("city", food.getString("City").length()==0?"null":food.getString("City"));
			map.put("town", food.getString("Town").length()==0?"null":food.getString("Town"));
			map.put("addr", food.getString("Address").length()==0?"null":food.getString("Address"));
			map.put("feature", food.getString("FoodFeature").length()==0?"null":food.getString("FoodFeature"));
			map.put("picurl", food.getString("PicURL").length()==0?"null":food.getString("PicURL"));
			foods[i]=map;
		}
		}catch (Exception e) {
